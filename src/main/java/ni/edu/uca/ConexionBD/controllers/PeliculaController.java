...
public class PeliculaController {
	...
	@PostMapping("/agregar")
	public String AgregarPel(@RequestBody Pelicula p) {
		String msg = "Error al guardar registro...";
		int b = servicio.GuardarRegistro(p);
		if(b==1) msg = "Registro guardado satisfactoriamente...";
		return msg;
	}
	
	@PutMapping("/actualizar/{id}")
	public String EditarPel(@RequestBody Pelicula p, @PathVariable int id, Model model) {
		String msg = "Error al editar registro...";
		//g.setIdGenero(id);
		int b = servicio.EditarRegistro(p);
